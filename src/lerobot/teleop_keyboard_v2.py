...
class SO101TeleopController:
    ...
    def __init__(self, robot, kinematics=None, keymap=None, joint_map=None):
        self.robot = robot
        self.kinematics = kinematics if kinematics else SO101Kinematics()
        self.keymap = keymap or {}
        self.joint_map = joint_map or {}  # NEW: mapping from generic to robot-specific joint names
    ...
    def handle_keyboard_input(self, key_state):
        """
        key_state: dict mapping semantic action names (e.g. 'shoulder_pan+', 'x+', 'reset') to bool (pressed or not)
        """
        if not key_state:
            return True
            
        for action, pressed in key_state.items():
            if not pressed:
                continue
            ...
            # Joint controls
            if action == 'shoulder_pan+':
                self.target_positions['shoulder_pan'] += 1
                print(f"shoulder_pan+ -> {self.target_positions['shoulder_pan']}")
            elif action == 'shoulder_pan-':
                self.target_positions['shoulder_pan'] -= 1
                print(f"shoulder_pan- -> {self.target_positions['shoulder_pan']}")
            elif action == 'wrist_roll+':
                self.target_positions['wrist_roll'] += 1
                print(f"wrist_roll+ -> {self.target_positions['wrist_roll']}")
            elif action == 'wrist_roll-':
                self.target_positions['wrist_roll'] -= 1
                print(f"wrist_roll- -> {self.target_positions['wrist_roll']}")
            elif action == 'gripper+':
                self.target_positions['gripper'] += 1
                print(f"gripper+ -> {self.target_positions['gripper']}")
            elif action == 'gripper-':
                self.target_positions['gripper'] -= 1
                print(f"gripper- -> {self.target_positions['gripper']}")

            # XY controls (planar movement)
            elif action == 'x+':
                self.current_x += 0.0021
    ...
    def execute_p_control_step(self):
        ...
        for joint_name, target_pos in self.target_positions.items():
            if joint_name in current_positions:
                ...
                # Use the mapping to get the correct XLerobot joint name
                mapped_name = self.joint_map.get(joint_name, joint_name)
                robot_action[f"{mapped_name}.pos"] = new_position
    ...
    def control_loop(self, keyboard):
        ...
        while True:
            try:
                ...
                # 执行P控制
                action  = self.execute_p_control_step()
